...
print("Ngrok authtoken set to mine after replacing")
...
# Terminatfd any existing ngrok tunnels to ensure a clean start
ngrok.kill()
print("Killed existing ngrok tunnels.")


streamlit_command = ["streamlit", "run", "streamlit_app.py", "--server.port", "8501", "--server.headless", "true"]
streamlit_process = subprocess.Popen(streamlit_command)
print(f"Streamlit process started in background with PID: {streamlit_process.pid}")
...
while time.time() - start_time < timeout:
    try:
        ...
        # If connection fails, Streamlit is not yet ready
        print("Waiting for Streamlit to start...")
    time.sleep(2) # Wait 2 seconds before retrying

if not streamlit_running:
    print("Streamlit server did not start in time. Terminating Streamlit process.")
    streamlit_process.terminate() # Attempt to terminate the Streamlit process
    raise Exception("Streamlit server failed to start within the allocated time.")

# it Open a ngrok tunnel to the Streamlit port
# it will expose the local Streamlit app to the internet.
